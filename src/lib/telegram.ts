...
export async function sendMessage(chatId: number | string, threadId: number, text: string): Promise<void> {
  ...
  log('[telegram:sendMessage]', { chatId, threadId })
}

export async function alertSistema(message: string): Promise<void> {
  const chatId = process.env.TELEGRAM_CHAT_ID!
  // topic 20 = sistema/alerts
  await sendMessage(chatId, 20, `🚨 <b>bodybasetwitter alert</b>\n\n${message}`)
}
